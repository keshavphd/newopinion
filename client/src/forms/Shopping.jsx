...
const Shopping = ({ onFormSubmit }) => {
  ...
  const {
    ...
    defaultValues: {
      decisinj: user.decisinj,
      incomi: user.incomi,
    },
  });

  const formdill4 = "abcdef";
  const onSubmit = async (formData) => {
    try {
      const saveData = await Axios({
        ...SummaryAPI.updateShopping,
        data: { ...formData, formdill4 },
      });
      if (saveData) {
  ...
  const optionra = [
    ...
    {
      value: "₹10,000,000,000.00 or above",
      label: "₹10,000,000,000.00 or above",
    },
  ];
        ...
        >
          ...
          <div className="">
            <label className="w-full text-sm">
              How Would You Best Describe Your Ability In Your Financial
              Decision Making <span className="text-sm text-red-500">*</span>
              <select
                {...register("decisinj", { required: "" })}
                className={`w-full ${
                  errors.decisinj ? "border-red-500" : "border-gray-300"
            ...
            <label className="w-full text-sm">
              ...
              <select
                {...register("incomi", { required: "" })}
                className={`w-full ${
                  errors.incomi ? "border-red-500" : "border-gray-300"
                } p-2.5 mb-2 text-base border rounded-md outline-none border-neutral-300 focus-within:border-amber-800`}
              >
                <option value="">Select an option</option>
